...
# Paragraph Length Optimization Rules
1. The UI provides a "Longitud de Diálogo / Narrador" control to optimize the word count of sentences/dialogue generated by the AI for videos.
2. The available optimization durations are 5s, 8s, 10s, and 15s (defaulting to 10s).
3. The AI MUST limit the length of generated dialogue/narrator expressions to approximately 2.5 words per second, ensuring the spoken text fits exactly within the requested video duration window.
4. This duration parameter is passed to the video prompt, image prompt, script, and podcast generation logic to enforce word limits.

# Download Filename Rules
1. When downloading generated text (Podcast, Script, Video Prompts, Image Prompts), the generated filename MUST include the duration in seconds used for optimization, appended as a suffix (e.g. `_10S.txt` or `_15S.txt`).
